...
def _is_fip_port(port):
    """Check whether the given port is an administrative port for
    floating IP creation
    """
    device_owner = port['device_owner']
    return (device_owner in l3_db.DEVICE_OWNER_FLOATINGIP)

def _is_vif_port(port):
    """Check whether the given port is a standard VIF port."""
    device_owner = port['device_owner']
    return (not _is_dhcp_port(port) and
            not _is_fip_port(port) and
            not _is_router_gw_port(port) and
            not _is_router_interface_port(port))
    ...
    def create_port(self, context, port):
        ...
        try:
            ...
            with session.begin(subtransactions=True):
                ...
                # if the network is an external network, then this ip needs
                # a route on the provider router to find this ip
                    ...
                    if net[ext_net.EXTERNAL]:
                        ...
                        for ip in port_data["fixed_ips"]:
                            self.client.add_router_route(
                                ...
                                dst_network_addr=ip["ip_address"],
                                dst_network_length=32,
                                next_hop_port=link_port.get_peer_id(),
                                weight=100)

                if not _is_fip_port(port_data):
